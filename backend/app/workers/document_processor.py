...
def process_document(self: Task, job_id: int, document_id: int):
    ...
    try:
        ...
        update_job_in_db(
            ...
            current_step="document_received",
            progress=2.0,
        )
        publish_progress(job_id, "document_received", 2.0, "Document received by worker")
        time.sleep(0.3)

        # ── Stage 2: Job Started ──────────────────────────────────────
        update_job_in_db(db, job_id, current_step="job_started", progress=5.0)
        ...
        # ── Stage 7: Final Result Stored ─────────────────────────────
        update_job_in_db(
            db, job_id,
            current_step="final_result_stored",
            progress=92.0,
            result=extracted,
        )
        publish_progress(
            job_id, "final_result_stored", 92.0,
            "Extracted result persisted to database",
